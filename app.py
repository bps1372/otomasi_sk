...
# 2. Input Data Tabel (Dinamis dengan Fitur Tambah Baris Otomatis)
st.subheader("F. Lampiran")

# Fitur Tambah Baris Otomatis
col_n1, col_n2 = st.columns([1, 3])
with col_n1:
    target_rows = st.number_input("Input jumlah baris yang diinginkan:", min_value=1, max_value=1000, value=1)

# Inisialisasi session state untuk tabel agar data tidak hilang saat rerun
...
# Logika Tombol Update Baris
if st.button(f"Generate {target_rows} Baris"):
...
# Editor Tabel
edited_df = st.data_editor(st.session_state.df_lampiran, num_rows="dynamic", use_container_width=True)
# Simpan perubahan editor kembali ke session state
...
if st.button("Proses & Buat Dokumen", type="primary"):
    if not tentang or not pelaksanaan:
        st.warning("Mohon lengkapi Informasi Umum terlebih dahulu.")
    else:
