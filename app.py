...
# 4. Menampilkan Preview & Tombol Unduh
if st.session_state.doc_ready:
    st.success("✅ Dokumen berhasil diproses!")
    
    # --- TAMBAHAN PREVIEW DOKUMEN ---
    st.subheader("👁️ Preview Dokumen")
    with st.expander("Klik untuk melihat intisari dokumen sebelum diunduh", expanded=True):
        st.info("💡 Catatan: Ini adalah preview teks draf. Format asli (tata letak halaman, margin, font) akan terlihat sempurna saat dibuka di Microsoft Word.")
        
        # Baca teks dari docx yang ada di memory
        preview_doc = Document(io.BytesIO(st.session_state.doc_data))
        
        # Tampilkan paragraf teks
        ...
        # Tampilkan tabel dataframe sebagai representasi isi lampiran
        st.dataframe(edited_df, use_container_width=True)
    # --- AKHIR TAMBAHAN PREVIEW ---

    st.download_button(
        label="⬇️ Unduh Dokumen Hasil (.docx)",
